...
module.exports=function studioRoutes(db,auth,access) {
 ...
 const publicTenant=async slug=>{
  ...
  const page=a=>({nome:a.barbearia_nome,descricao:a.page_description,telefone:a.telefone,slug:a.public_slug || 'studio-'+a.id,capa:a.cover_image,logo:a.logo_image,cor:a.page_color});
 ...
 router.post('/public/:slug/agendamentos',wrap(async(req,res)=>{
  const a=await publicTenant(req.params.slug);
  res.status(201).json(await tx(s=>s.book(a.id,req.body)));
 }));
 router.use(auth);
 router.get('/painel',wrap(async(req,res)=>{
  ...
  const result=await tx(async(s,c)=>{
   ...
   return {pagina:page(a),horarios:effectiveHours(a),servicos:await s.services(id),profissionais:await s.professionals(id),
 ...
 router.post('/servicos',wrap(async(req,res)=>{
  ...
  const result=await tx(async(st,c)=>{
   ...
   const r=await c.runAsync('INSERT INTO servicos_assinatura (assinatura_id,nome,descricao,preco,duracao,foto,ativo) VALUES ($1,$2,$3,$4,$5,$6,$7)',[req.assinatura.id,s.nome,s.descricao,s.preco,s.duracao,s.foto,s.ativo]);
 ...
 router.put('/servicos/:id',wrap(async(req,res)=>{
  const s=await serviceInput(req.body);
  const r=await db.runAsync('UPDATE servicos_assinatura SET nome=$1,descricao=$2,preco=$3,duracao=$4,foto=$5,ativo=$6 WHERE id=$7 AND assinatura_id=$8',[s.nome,s.descricao,s.preco,s.duracao,s.foto,s.ativo,req.params.id,req.assinatura.id]);
 ...
 router.put('/pagina',wrap(async(req,res)=>{
  ...
  await db.runAsync('UPDATE assinaturas SET barbearia_nome=$1,page_description=$2,telefone=$3,public_slug=$4,page_color=$5,cover_image=$6,logo_image=$7 WHERE id=$8',[nome,descricao,telefone,slug,b.cor,capa,logo,req.assinatura.id]);
  res.json({ok:true});
 }));
